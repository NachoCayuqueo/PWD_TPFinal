...
function crearTablaRoles($listaRoles)
{
    ...
    foreach ($listaRoles as $rol) {
        ...
        $disabled = $fechaEliminacion  ? "disabled" : "";

        echo "<tr>";
        echo "<td class='card-title'>" . $rol->getIdRol() . "</td>";
        echo "<td>" . $rol->getRoDescripcion() . "</td>";
        echo "<td class='text-center'>" . dateFormat($fechaEliminacion)  . "</td>";
        echo "<td class='text-center'>
                <a href='#' class='btn btn-outline-primary edit-btn " . $disabled . "' data-bs-toggle='modal' data-bs-target='#modalEdit_" . $rol->getIdRol() . "' type='button' data-bs-tooltip='tooltip' data-bs-placement='left' data-bs-title='Editar' >
                    <img src='" . $GLOBALS['BOOTSTRAP_ICONS'] . "/pen.svg' alt='edit'>
                </a>
                <a href='#' class='btn btn-outline-danger delete-btn " . $disabled . "' data-bs-toggle='modal' data-bs-target='#modalDelete_" . $rol->getIdRol() . "' type='button' data-bs-tooltip='tooltip' data-bs-placement='right' data-bs-title='Borrar'>
